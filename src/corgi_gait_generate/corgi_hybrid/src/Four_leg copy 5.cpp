...
{ 
    ...
    // 2) 前進半步長
    // std::cout << "duty: " << robot.legs[leg_index].duty << std::endl;
    double move_duty = (0.5 * robot.legs[leg_index].step_length_current)-robot.legs[leg_index].step_length_current*robot.legs[leg_index].duty;
    // std::cout << "move_duty: " << move_duty << std::endl;
    if (move_duty < 0) {
        for (double s = 0; s < -move_duty; s += 0.001) {
            try {
                eta = leg.move(eta[0], eta[1], {0.001, 0.0}, 0.0);
            } catch (std::runtime_error &e) {
                return UN_CONVERGED;
...
void optimizePose(Robot &robot, LegModel &leg_model, bool swing_index) {
    ...
    for(int i = 0; i < 4; ++i) {
        ...
        for (double h = 0.34; h >= 0.12; h -= 0.0005){
            bool theta_bound_reached = false;
...
int main(int argc, char** argv) {
    ...
    double initial_SL = 0.15; // 初始步長
    // A. Create terrain
    double terrain_start_height = -0.149;
    Terrain terrain_zigzag(terrain_start_height);
    // terrain_zigzag.addPlain(0.93982);     
    // in tan() ___/\/\/\/\_________ 
    terrain_zigzag.addSlope((2.23716094422), 0.04080820618); 
    terrain_zigzag.addSlope(-(2.23716094422), 0.04080820618);  
    terrain_zigzag.addSlope((2.23716094422), 0.04080820618); 
    terrain_zigzag.addSlope(-(2.23716094422), 0.04080820618); 
    terrain_zigzag.addSlope((2.23716094422), 0.04080820618); 
    ...
    while(robot.body_position[0]<2.5){
